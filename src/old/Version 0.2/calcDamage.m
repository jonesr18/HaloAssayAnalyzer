...
function results = calcDamage(imRegions, pairs, varargin)
    ...
    for n = 1:length(imRegions)
        ...
        % Calculate NDF for each per halo-nucleus pair
        if getNDF
        ...
        if getMoment || getAdjMom || getDist
            ...
            % Conisder doing this by subtracting images instead -- should be much faster
            haloPixels = struct;
            for i = 1:numPairs
                if moreHalos
            ...
            % Calculate halo moment, adjusted halo moment, and/or halo distance
            if getMoment
...
function [showStats, getNDF, getMoment, getAdjMom, getDist] = parseInputs(imRegions, pairs, args)
    ...
    if nargin > 1
        if ~isempty(args)
            for arg = args                      % arg is a cell, must make it non-cell later
                if islogical(arg{1})
                    showStats = arg{1};
                elseif ischar(arg{1})
                    if first
                    ...
                    if strcmpi(arg{1}, 'all')
                        getNDF    = true;
                        getMoment = true;
                        getAdjMom = true;
                        getDist   = true;
                        break
                    elseif strcmpi(arg{1}, 'NDF')
                        getNDF = true;
                    elseif strcmpi(arg{1}, 'moment')
                        getMoment = true;
                    elseif strcmpi(arg{1}, 'adjMoment')
                        getAdjMom = true;
                    elseif strcmpi(arg{1}, 'distance')
                        getDist = true;
                    else
                        error('Capstone:ArgChk', 'Test name not recognized: %s.', arg{1})
                    end
                else
                    error('Capston:ArgChk', 'Optional arguments must be of type boolean or string.')
                end
            end
        ...
        if iscell(pairs)
            if size(pairs, 2) ~= 3
                error('Capstone:ArgChk', 'Cell array ''pairs'' must contain 3 columns.')
            end
            for row = pairs'
                if ~islogical(row{1})
